      ...
      {products.map((product)=>{
        ...
        return <article key={productId} className="group relative">
          ...
          <Link href={`/products/${productId}`}>
            {/* This is the Card that is the one element in the grid and grid contains a lot of cards */}
            <Card className="transform group-hover:shadow-xl transition-shadow duration-500">
              <CardContent className="pt-4">
                <div className="relative h-64 md:h-48 rounded overflow-hidden">
                  <Image src={image} alt={name} fill sizes='(max-width: 768px) 100vw, (max-width:1200px) 50vw, 33w' priority
